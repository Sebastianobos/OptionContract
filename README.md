...
## Buying options
The function `buy(uint256 a)` buys an amount a of option contracts at the conditions specified in the smart contract. The option price is paid to the smart contract. Once `buy(uint256 a)` is called no more trading can happen. The option expirer after the specified duration (in seconds) has passed from when they were bought. The payoff of the option, that is `max(0, underlyingPrice-strike)` can be checked with `getValue()`.
